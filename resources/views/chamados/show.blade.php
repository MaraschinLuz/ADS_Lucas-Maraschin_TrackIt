<x-app-layout>
    ...
    <div class="mt-6 p-4 bg-white dark:bg-gray-800 rounded shadow">
        <div class="flex items-center justify-between mb-2">
            <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-100">Chat do chamado</h3>
        ...
        <form method="POST" action="{{ route('mensagens.store', $chamado) }}" class="mb-3">
            @csrf
            <textarea name="mensagem" rows="3" class="w-full rounded border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200" placeholder="Escreva uma mensagem..."></textarea>
            <div class="mt-2 text-right">
                <button class="px-4 py-2 rounded bg-indigo-600 text-white">Enviar</button>
            </div>
        </form>

        <div class="space-y-2 max-h-96 overflow-y-auto">
            @forelse($chamado->mensagens->sortBy('created_at') as $msg)
                <div class="p-3 rounded border border-gray-200 dark:border-gray-700">
                    <div class="text-sm text-gray-500 dark:text-gray-400">
                        {{ $msg->created_at->format('d/m/Y H:i') }} • {{ $msg->user->name ?? 'Usuário removido' }}
                    </div>
                    <div class="text-gray-800 dark:text-gray-100 whitespace-pre-line">{{ $msg->mensagem }}</div>
                </div>
            @empty
                <div class="text-sm text-gray-500">Nenhuma mensagem ainda.</div>
            @endforelse
        </div>
    </div>
